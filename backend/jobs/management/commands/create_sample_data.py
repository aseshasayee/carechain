...
class Command(BaseCommand):
    help = 'Create sample data for testing the applicant tracking system'
    ...
    def handle(self, *args, **options):
        ...
        if created:
            ...
            recruiter_user.save()
            self.stdout.write(f"Created recruiter user: {recruiter_user.email}")
        ...
        if created:
            self.stdout.write(f"Created recruiter profile")
        ...
        for candidate_data in candidates_data:
            ...
            if created:
                ...
                user.save()
                self.stdout.write(f"Created candidate user: {user.email}")
            ...
            if created:
                self.stdout.write(f"Created candidate: {candidate_profile.first_name} {candidate_profile.last_name}")
        
        # Create sample job applications
        applications_data = [
            # Applications for ICU Nurse job
            {'candidate_idx': 0, 'job_idx': 0, 'status': 'applied'},
            {'candidate_idx': 1, 'job_idx': 0, 'status': 'in_review'},
            # Applications for Cardiologist job
            {'candidate_idx': 2, 'job_idx': 1, 'status': 'applied'},
            {'candidate_idx': 3, 'job_idx': 1, 'status': 'interviewed'},
            # Applications for Emergency job
            {'candidate_idx': 4, 'job_idx': 2, 'status': 'applied'}
        ]
        ...
        for app_data in applications_data:
            ...
            if created:
                self.stdout.write(f"Created application: {candidate.first_name} {candidate.last_name} -> {job.title}")
        
        self.stdout.write(self.style.SUCCESS("Sample data creation completed!"))
        self.stdout.write(f"Total Jobs: {Job.objects.count()}")
        self.stdout.write(f"Total Applications: {JobApplication.objects.count()}")
